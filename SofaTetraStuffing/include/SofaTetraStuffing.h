...
public:
   ...
   * Additionally we will set all necessary parameters for Shewchuk's
   * algorithm.
   */
  void GenerateFromSurface(const std::vector<Triangle> &tris,
                           const std::vector<Vec3f> &verts, const float size_,
                           const float as_, const float al_,
                           const bool snapToSurface, const bool splitTetras);
