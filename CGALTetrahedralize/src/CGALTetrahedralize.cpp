...
typedef CGAL::Mesh_complex_3_in_triangulation_3<
    Tr>::Vertices_in_complex_iterator Complex_Vertex_Iterator;
typedef CGAL::Mesh_complex_3_in_triangulation_3<Tr>::Cells_in_complex_iterator
    Complex_Cell_Iterator;
...
struct Copy_polyhedron_to
  ...
  void operator()(typename Polyhedron_output::HalfedgeDS &out_hds) {
    ...
    typedef
        typename Polyhedron_input::Vertex_const_iterator Vertex_const_iterator;
    typedef
        typename Polyhedron_input::Facet_const_iterator Facet_const_iterator;
    typedef
        typename Polyhedron_input::Halfedge_around_facet_const_circulator HFCC;
...
/*
 * This is used to create the CGAL compliant surface mesh from the list of
 * vertices and triangle indices:
 *
 * The following section was found in the CGAL documentation for
...
public:
  Build_triangle(const std::vector<Triangle> &tris_,
  ...
  void operator()(HDS &hds) {
    if (tris.size() == 0 || verts.size() == 0) {
      std::cerr
          << "ERROR in CGALTetrahedralize! Vertices or triangles are empty..."
          << std::endl;
      return;
    }
...
CGALTetrahedralize::~CGALTetrahedralize() { clear(); }

/// Helpful documentation note for self:
/// http://doc.cgal.org/latest/Mesh_3/index.html#Chapter_3D_Mesh_Generation

void CGALTetrahedralize::GenerateFromSurface(
    const std::vector<Triangle> &tris, const std::vector<Vec3f> &verts,
    const double cell_size_, const double facet_angle_,
    const double facet_size_, const double face_distance_,
    const double cell_radius_edge_ratio_) {
  std::cout << "Generating CGAL surface mesh from our own data structure..."
            << std::endl;
  ...
  // transform the generated mesh from exact to inexact mesh criteria (this is
  // required to generate the tetrahedral mesh)
  tmpPoly.delegate(triangle);
  // CGAL_assertion( polyhedron.is_triangle( polyhedron.halfedges_begin()));

  // std::cout<<"Successfully generated CGAL mesh from custom
  // surface..."<<std::endl;

  // std::cout<<"Copying CGAL surface to new structure to generate tetrahedral
  // mesh mesh from surface..."<<std::endl;
  ...
  Mesh_criteria criteria(facet_angle = facet_angle_, facet_size = facet_size_,
                         facet_distance = face_distance_,
                         cell_radius_edge_ratio = cell_radius_edge_ratio_,
                         cell_size = cell_size_);
  // Mesh_criteria criteria(cell_size=0.1, cell_radius_edge_ratio=3);

  // Mesh generation
  std::cout << "Making mesh (this might take a while depending on the size of "
               "the surface mesh...)"
            << std::endl;
  C3t3 c3t3 =
  ...
  /*
    ...
    std::cout<<"C3T3 Number of cells after refining:
  ...
  */
  ...
  i = 0;
  //  Vertex map for storing the vertex indices (these are needed to generate
  //  the triangle indices from the vertex values)
  ...
  for (Finite_vertices_iterator it = t.finite_vertices_begin();
    ...
    // add the current point to the vertex map to re-use this map to generate
    // the triangle-indices afterwards.
    V[it->point()] = i;
    Vec3f v;
    v.x = it->point().x();
    v.y = it->point().y();
  ...
  i = 0;
  for (Complex_Cell_Iterator it = c3t3.cells_in_complex_begin();
