...
QuartetTetraStuffingDialog::QuartetTetraStuffingDialog(QWidget *parent)
    : BaseTetraDialog(parent), m_ui(new Ui::QuartetTetraStuffingDialog) {
  m_ui->setupUi(this);
  _viewer = NULL;
}

QuartetTetraStuffingDialog::~QuartetTetraStuffingDialog() { delete m_ui; }

void QuartetTetraStuffingDialog::setParams(const float tetraSize_,
...
void QuartetTetraStuffingDialog::generateTetras() {
  ...
  cdt.GenerateFromSurface(tmpTris, tmpVerts, m_ui->gridSpacingSpinBox->value(),
                          m_ui->angleThresholdSpinBox->value(), true);
  ...
  ss << "Generated tetrahedral mesh in " << t.elapsed()
     << "ms. Tetras: " << cdt.GetTetras().size()
     << "; Vertices: " << cdt.GetTetraVertices().size();
