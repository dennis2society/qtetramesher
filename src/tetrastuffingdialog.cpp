...
TetraStuffingDialog::TetraStuffingDialog(QWidget *parent)
    : BaseTetraDialog(parent), m_ui(new Ui::TetraStuffingDialog) {
  m_ui->setupUi(this);
  _viewer = NULL;
}

TetraStuffingDialog::~TetraStuffingDialog() { delete m_ui; }
...
void TetraStuffingDialog::generateTetras() {
  ...
  sts->GenerateFromSurface(tris, verts, m_ui->tetraSizeSpinBox->value(),
                           m_ui->alphaShortSpinBox->value(),
                           m_ui->alphaLongSpinBox->value(),
                           m_ui->snapToPointsCheckBox->isChecked(),
                           m_ui->splitTetrasCheckBox->isChecked());
