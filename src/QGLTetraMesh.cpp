    ...
    if (isReady)
	{
    	...
    	switch (tetraMode)
    	{
            ...
            case 1:	// wireframe
				if (top != NULL)
				{
                    glColor3f(tetraColorWireframe.x, tetraColorWireframe.y, tetraColorWireframe.z); /// yellow wireframe
					glDisable(GL_LIGHTING);
					glDisable(GL_CULL_FACE);
					glVertexPointer(3, GL_FLOAT, sizeof(Vec3f), &(top->GetVertices()[0]));
					// draw triangles
					glEnableClientState (GL_VERTEX_ARRAY);
					glDrawElements(GL_LINES, top->GetNumEdges()*2, GL_UNSIGNED_INT, &(top->GetEdges()[0]));
...
//				    glEnable(GL_CULL_FACE);
					...
					for (unsigned int i=0; i<tetras.size(); ++i)
					{
						...
						float maxZ = vertices[t.index[0]].z;
						for (unsigned int j=1; j<4; ++j)
						{
							if (vertices[t.index[j]].z > maxZ)
								maxZ = vertices[t.index[j]].z;
						}
						//const Vec3f& v0 = vertices[t.index[0]];
            ...
            */
            case 0:	// solid tetras with cut plane
				if (top != NULL)
				{
					...
					{
						const Tetrahedron& t = tetras[i];
						Vec3f centroid = t.Centroid(vertices[t.index[0]], vertices[t.index[1]], vertices[t.index[2]], vertices[t.index[3]]);
						...
						if (maxZ <= (b.max.z - cutDist))
						{
							...
							glVertex3f(centroid.x+vc0.x, centroid.y+vc0.y, centroid.z+vc0.z);
							glVertex3f(centroid.x+vc3.x, centroid.y+vc3.y, centroid.z+vc3.z);
							glVertex3f(centroid.x+vc2.x, centroid.y+vc2.y, centroid.z+vc2.z);
                            normal = (vc1 - vc3).cross((vc2 - vc3));
							normal.normalize();
							glNormal3f(normal.x, normal.y, normal.z);
							glVertex3f(centroid.x+vc3.x, centroid.y+vc3.y, centroid.z+vc3.z);
							glVertex3f(centroid.x+vc1.x, centroid.y+vc1.y, centroid.z+vc1.z);
							glVertex3f(centroid.x+vc2.x, centroid.y+vc2.y, centroid.z+vc2.z);
    	...
    	switch (surfaceMode)
    	{
			...
			case 1: // wireframe
				if (surf != NULL)
				{
                    glColor3f(surfaceColorWireframe.x, surfaceColorWireframe.y, surfaceColorWireframe.z); /// yellow wireframe
					glDisable(GL_LIGHTING);
					glDisable(GL_CULL_FACE);
					glVertexPointer(3, GL_FLOAT, sizeof(Vec3f), &(surf->GetVertices()[0]));
					// draw triangles
					glEnableClientState (GL_VERTEX_ARRAY);
					glDrawElements(GL_LINES, surf->GetNumEdges()*2, GL_UNSIGNED_INT, &(surf->GetEdges()[0]));
					glDisableClientState (GL_VERTEX_ARRAY);
				}
				break;
            case 0:	// surface with normals
				if (surf != NULL)
				{
					...
					glEnable(GL_LIGHTING);
					glEnable(GL_CULL_FACE);
                    glColor3f(surfaceColorSolid.x, surfaceColorSolid.y, surfaceColorSolid.z); /// grey
					glVertexPointer(3, GL_FLOAT, sizeof(Vec3f), &(surf->GetVertices()[0]));
					glNormalPointer(GL_FLOAT, sizeof(Vec3f), &(surf->GetNormals()[0]));
					// draw triangles
					glEnableClientState (GL_VERTEX_ARRAY);
					glEnableClientState(GL_NORMAL_ARRAY);
					glDrawElements(GL_TRIANGLES, surf->GetNumTriangles()*3, GL_UNSIGNED_INT, &(surf->GetTriangles()[0]));
...
void QGLTetraMesh::DrawTetraTriangle(const unsigned int triangleIndex, const Vec3f& tetraCentroid, const Vec3f& color)
{
	const Triangle& t = top->GetTriangles()[triangleIndex];
	// funny way to calculate the normal from the distance from the tetrahedron's centroid to the triangle's centroid
	glBegin(GL_TRIANGLES);
	...
	normal.normalize();
	glNormal3f(normal.x, normal.y, normal.z);
	glVertex3f(tetraCentroid.x + v0.x, tetraCentroid.y + v0.y, tetraCentroid.z + v0.z);
	//glNormal3f(normal.x, normal.y, normal.z);
	glVertex3f(tetraCentroid.x + v1.x, tetraCentroid.y + v1.y, tetraCentroid.z + v1.z);
	//glNormal3f(normal.x, normal.y, normal.z);
	glVertex3f(tetraCentroid.x + v2.x, tetraCentroid.y + v2.y, tetraCentroid.z + v2.z);
	glEnd();
}
...
void QGLTetraMesh::DrawTetrahedron(const unsigned int tetraIndex)
{
	...
	Vec3f centroid = Tetrahedron::Centroid(top->GetVertices()[t.index[0]], top->GetVertices()[t.index[1]], top->GetVertices()[t.index[2]], top->GetVertices()[t.index[3]]);
...
bool QGLTetraMesh::SaveSurface(const std::string& fileName_)
{
    ...
    bool success = twriter.writeFile(fileName_, surf->GetVertices(), surf->GetTriangles());
    t.stop();
    std::cout<<"Finished saving Surface in "<<t.getElapsedTimeInMilliSec()<<" ms."<<std::endl;
    return success;
}
