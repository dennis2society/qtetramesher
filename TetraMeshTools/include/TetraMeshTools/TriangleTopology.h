...
protected:
  ...
  std::vector<EdgeVertex>
      _edgeVertices; /// edges-per-vertex list, 1..n entries per vertex
  std::vector<PrimitivesPerVertex>
      _vertexEdgesLookup; /// contains one entry per vertex
  std::vector<TriangleVertex>
      _triangleVertices; /// triangles-per-vertex list, 1..n entries per vertex
  std::vector<PrimitivesPerVertex>
      _vertexTrianglesLookup; /// contains one entry per vertex
  std::vector<TriangleEdges>
      _triangleEdges; /// the list of edges-per-triangle, 1 struct per triangle
...
public:
  ...
  TriangleTopology(const std::vector<Vec3f> &vertices_,
  ...
  /**
   * Initializes the triangle-mesh from a list of vertices and a list of
   * triangle-indices. The fullUpdate flag indicates if the initialization
   * should re-generate all data-structures. The default for fullUpdate_ is to
   * only update vertices and triangle-indices. All other lists will be created
   * when they are being accessed for the first time.
   */
  virtual void Init(const std::vector<Vec3f> &vertices_,
                    const std::vector<Triangle> &triangles_,
                    const bool fullUpdate_ = false);

  /**
   * Set Edges separately (e.g. when Edges have been generated from the
   * tetrahedra already. This will also clear any edge-related mappings.
   */
  ...
  unsigned int GetNumVertices() const { return _vertices.size(); }

  unsigned int GetNumEdges() const { return _edges.size(); }

  unsigned int GetNumTriangles() const { return _triangles.size(); }
   ...
   */
  BoundingCube GetBoundingCube() { return bc; }
   ...
   */
  BoundingCube GetLargeBoundingCube() { return lbc; }
