 ...
 * Contains the min point (min x, min y, minz) and max point (max x, max y, max
 * z)
 */
struct BoundingBox {
  Vec3f min;
...
/**
 * Struct for an axis-aligned bounding cube as a basis for an octree and BCC
 * (Body Centered Cubic). Contains the size that will be used for width, height
 * and length.
 */
struct BoundingCube {
  float size;
...
public:
  ...
  Triangle(const unsigned int i0, const unsigned int i1,
  ...
  inline bool operator==(const Triangle &t) const {
    return ((index[0] == t.index[0]) && (index[1] == t.index[1]) &&
            (index[2] == t.index[2]));
  }
...
public:
  ...
  inline bool operator==(const Tetrahedron &t) const {
    return ((index[0] == t.index[0]) && (index[1] == t.index[1]) &&
            (index[2] == t.index[2]) && (index[3] == t.index[3]));
  }
