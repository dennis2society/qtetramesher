...
bool TetraTools::TriMeshLoader::loadFile(const std::string& path_and_filename)
{
    ...
    const aiScene* tm = importer.ReadFile(path_and_filename, aiProcess_Triangulate            |
	...
	{
        std::cout<<"Successfully loaded surface mesh with "<<mesh->mNumVertices<<" vertices and "<<mesh->mNumFaces<<" faces."<<std::endl;
	}
    ...
    for (unsigned int i=0; i<mesh->mNumFaces; ++i)
	{
        ...
        if (f.mNumIndices != 3) {
            std::cerr<<"Error in TriMeshLoading! Loaded shape has more than three indices!"<<std::endl;
            return false;
        }
		Triangle t;
