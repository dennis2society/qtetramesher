...
bool TetraTools::TetgenWriter::SaveToFile(
    const std::string &fileName_, const std::vector<Vec3f> &verts,
    const std::vector<Tetrahedron> &tetras) {
  std::ofstream nodeFile;
  std::string nodeFilename = fileName_ + std::string(".node");
  nodeFile.open(nodeFilename.c_str());
  if (nodeFile.is_open()) {
    nodeFile << "# Node count, 3 dim, no attribute, no boundary marker"
             << std::endl;
    ...
    if (eleFile.is_open()) {
      eleFile << "# Number of tetrahedra, number of indices per line, 0"
              << std::endl;
      ...
      for (unsigned int i = 0; i < tetras.size(); ++i) {
        ...
        eleFile << (i + 1) << " " << (t.index[0] + 1) << " " << (t.index[1] + 1)
                << " " << (t.index[2] + 1) << " " << (t.index[3] + 1)
                << std::endl;
      }
    ...
    {
            const Tetrahedron& t = tetras[i];
            outputFile<<(i+1)<<" 4 1 1 4 "<<(t.index[0]+1)<<"
    "<<(t.index[1]+1)<<" "<<(t.index[2]+1)<<" "<<(t.index[3]+1)<<std::endl;
    }
