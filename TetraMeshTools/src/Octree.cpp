...
Octree::Octree(const unsigned int maxDepth_, std::vector<Vec3f> *inPoints_,
...
void Octree::generateBoundingCube() {
  ...
  _minBC = Vec3f(lbc.center.x - maxHalf, lbc.center.y - maxHalf,
                 lbc.center.z - maxHalf);
  _maxBC = Vec3f(lbc.center.x + maxHalf, lbc.center.y + maxHalf,
                 lbc.center.z + maxHalf);
