...
int planeBoxOverlap(const Vec3f &normal, const Vec3f &vert,
                    const Vec3f &maxbox) {
  unsigned int q;
  Vec3f vmin, vmax;
  float v;
...
int triBoxOverlap(const Vec3f &boxcenter, const Vec3f &boxhalfsize_,
                  const Vec3f triverts[3]) {
  ...
  if (min > boxhalfsize_[0] || max < -boxhalfsize_[0])
    return 0;

  // test in Y-direction
  findMinMax(v0[1], v1[1], v2[1], min, max);
  if (min > boxhalfsize_[1] || max < -boxhalfsize_[1])
    return 0;

  // test in Z-direction
  findMinMax(v0[2], v1[2], v2[2], min, max);
  if (min > boxhalfsize_[2] || max < -boxhalfsize_[2])
    return 0;
  ...
  if (!planeBoxOverlap(normal, v0, boxhalfsize_))
    return 0;
