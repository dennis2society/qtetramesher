 ...
 *	This implementation is taken from https://github.com/crawforddoran/quartet
 *
...
/**
 *  Simple temporary geometry structs to avoid naming clashes of TetraMeshTools::Vec3f
 *  and the used tetra-meshing implementation.
 *  This requires conversion to and from the temporary types to be used in an application
 *  that uses a Vec3f/Vec4i type:
...
public:
	 ...
	 *	Additionally we will set all necessary parameters for this implementation of Shewchuk's algorithm.
   *  Also the output data containers will be filled here after processing.
	 */
	void GenerateFromSurface(const std::vector<inTriangle>& tris, const std::vector<tmpVec3f>& verts, float gridSpacing, float angleThreshold, bool optimize);

  /**
   *  Returns the generated tetrahedra vertices
   *  This is using a temporary Vec3f type to avoid conflicts with Crawford's vec.h declarations!
   *  This must be converted back to TetraMeshTools::Vec3f
   */
	std::vector<tmpVec3f>& GetTetraVertices();

  /**
   *  Returns the generated tetrahedra tetra indices
   *  This is using a temporary Tetrahedron type to avoid conflicts with Crawford's vec.h declarations!
   *  This must be converted back to TetraMeshTools::Tetrahedron
   */
