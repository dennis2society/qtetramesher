...
void TetStuffer::stuff(const std::vector<Vec3f> &surfVerts,
  ...
  if (surfVerts.empty() || surfTris.empty()) {
    std::cout << "GenericIsosurfaceStuffing: empty input\n";
    return;
  }
  ...
  std::cout << "GenericIsosurfaceStuffing: BBox [" << bmin.x << "," << bmin.y << "," << bmin.z
            << "] - [" << bmax.x << "," << bmax.y << "," << bmax.z << "]\n";
  std::cout << "GenericIsosurfaceStuffing: Grid spacing = " << gridSpacing
  ...
  if (graded) {
    ...
    std::cout << "GenericIsosurfaceStuffing: Lattice " << lattice.ni << " x " << lattice.nj
    ...
    std::cout << "GenericIsosurfaceStuffing: SDF evaluated at " << lattice.vertices.size() << " vertices\n";

    lattice.buildTets();
    std::cout << "GenericIsosurfaceStuffing: " << lattice.tets.size() << " lattice tetrahedra\n";
  }

  // 5. Apply warping rules
  Warp warp(lattice, sdf, alphaShort, alphaLong);
  std::cout << "GenericIsosurfaceStuffing: " << warp.warpedVertices.size() << " vertices warped, "
  ...
  std::cout << "GenericIsosurfaceStuffing: Output: " << outVerts.size() << " vertices, "
